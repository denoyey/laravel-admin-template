...
@php
    $title = 'Edit Role';
    $pageTitle = 'Edit Role';
    $pageSubtitle = 'Ubah role dan hak aksesnya.';
@endphp
...
@section('main-content')
    ...
    <div class="mb-6">
        <div class="space-y-3">
            ...
            <h1 class="text-base sm:text-2xl font-bold text-gray-900 capitalize">Edit {{ $role->name }}</h1>
        </div>
    </div>
            ...
            <div class="bg-white border border-gray-200 rounded-md shadow-sm p-4 sm:p-5 flex flex-col xl:flex-row xl:items-center justify-between gap-4 sm:gap-6">
                <div class="flex-1 w-full xl:max-w-xl">
                    <x-admin.forms.form-input
                        ...
                        :hint="$role->name === 'super_admin' ? 'Nama role super_admin bawaan sistem tidak dapat diubah.' : ''"
                        :hint-class="$role->name === 'super_admin' ? 'text-oren-dark' : 'text-gray-500'"
                    />
                </div>
